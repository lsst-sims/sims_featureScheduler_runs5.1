...
# Can update Scripted survey so the check is mounted or current.
class ScriptedSurveyFilter(ScriptedSurvey):
    """Modify scripted survey so it only provides observations if the filter is loaded"""
        ...
        if self.obs_wanted.size > 0:
            ...
            if np.size(in_time_window) > 0:
                ...
                if self.check_band_mounted:
                    ...
                    # Change to filter is loaded, not just mounted
                    match2 = np.isin(self.obs_wanted["band"][matches], conditions.current_filter)
            ...
            if np.size(matches) > 0:
                ...
                # functions
                reward = 0
                for baf, weight in zip(self.basis_functions, self.basis_weights):
                    basis_value = baf(conditions)
