...
def gen_roman_on_season(
    ...
    warnings.warn(
        "Generating Roman survey place holder. Should probably not be in production."
    )
    ...
    basis_functions.append(
        bf.ForceDelayBasisFunction(days_delay=0.8, scheduler_note=scheduler_note)
    )
    ...
    basis_functions.append(
        bf.MoonDistPointRangeBasisFunction(RA, dec, moon_limit=moon_limit)
    )
    basis_functions.append(bf.AirmassPointRangeBasisFunction(RA, dec, nside=nside))
    basis_functions.append(
        bf.AvoidDirectWind(wind_speed_maximum=wind_speed_maximum, nside=nside)
    )
...
def gen_roman_off_season(
    ...
    basis_functions.append(
        bf.ForceDelayBasisFunction(days_delay=3.0, scheduler_note=scheduler_note)
    )
    # Force it to be in a given observing season
    basis_functions.append(
